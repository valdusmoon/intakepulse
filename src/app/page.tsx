...
/**
 * /v4 — LANDING REDESIGN MOCK (image-heavy rebuild). Every section carries a
 * visual (photo with real HTML text overlaid + floating UI chips, a diagram, or
 * a light animation) so the page reads as professionally built, not template.
 * Follows the ReviewHarvest content spine — hero → pain (text-over-photo) →
 * mechanism → CTA → case studies → timeline → feature bento → comparison →
 * pricing → FAQ → animated closing CTA — with our bright treatment, real AI
 * photos, and NO founding-rate / NO fabricated proof (logo strip = the flagged
 * fictional placeholder). Motion is CSS + IntersectionObserver, not heavy JS.
 */
...
export default function V4Page() {
  return (
    <div className="min-h-screen bg-white text-[#152033]">
          ...
          <div className="relative max-w-6xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-[1fr_0.95fr] gap-10 items-center pt-32 pb-20">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 backdrop-blur px-3.5 py-1.5 text-xs font-semibold text-white mb-6">
                <span className="h-1.5 w-1.5 rounded-full bg-landing-primary-glow" /> Missed-call lead recovery for home-service trades
              </span>
      ...
      <section className="px-6 py-20 sm:py-28 bg-white">
        <div className="max-w-6xl mx-auto">
          <ScrollReveal className="max-w-2xl mb-12">
            ...
            <h2 className="font-cv-heading text-4xl sm:text-[52px] font-bold tracking-[-0.035em] leading-[1.02] mb-4">Right now, the call just gets lost.</h2>
            <p className="text-[16px] text-[#667085] leading-relaxed">Every missed call is a job that went to voicemail, or to the next company on the list.</p>
          </ScrollReveal>
        ...
        <ScrollReveal className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-12 items-start mb-14">
            <div>
              ...
              <h2 className="font-cv-heading text-4xl sm:text-[46px] font-bold tracking-[-0.035em] leading-[1.03] mb-4">One missed call, back as a ranked job.</h2>
              <p className="text-[15px] text-[#667085] leading-relaxed">Callverted takes over the same call your team couldn&apos;t reach, and turns it into a lead you can act on.</p>
            </div>
      ...
      {/* ── Not a receptionist. A recovery service. ─────────────────────── */}
