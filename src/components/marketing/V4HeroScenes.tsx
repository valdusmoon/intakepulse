...
/**
 * V4HeroScenes — the animated hero for /v4.
 *
 * Same three-beat product story as the live homepage's <HeroSceneCard/> (call →
 * intake → ranked lead) and the same beat durations, so the two heroes stay in
 * narrative sync. Everything else is deliberately different:
 *
 *   homepage          this
 *   ────────────────  ──────────────────────────────────────────────
 *   dark glass panel  light app UI
 *   360px, floats     full width, inside browser chrome
 *   over a photo      on white
 *   phone-shaped      dashboard-shaped (rail + top bar + content)
 *
 * The reason is /v4's thesis: the page bets on SHOWING THE SOFTWARE, and it is
 * all-light so product UI never sits on a background that fights it. A dark
 * floating panel would break both. Rendering the story as the app itself is
 * more on-thesis than reusing the homepage component would have been.
 *
