...
/**
 * V8SeeItWork — the redesigned "See It Work" centerpiece (v8 hybrid, §5).
 *
 * Three tabs, each a MATERIALLY DIFFERENT intake experience on the left:
 ...
 *                       transcript between the employee and caller; Callverted is not a speaker)
 ...
 *
 * The right-side OUTPUT PACKET is structurally IDENTICAL across all three tabs — same
 * "Lead record" field list, same "Outcome" block (Job pill + tier badge + emphasized
 * score + recommended action + context badge). Only the values change, plus the presence
 * of the numeric Score line (omitted on the website tab). That is how the section
 * demonstrates the architecture: many ways in, one ranked list out.
 *
 * Autoplays the active tab once on scroll-in (IntersectionObserver). Switching tabs, or
 * the visible Replay button, resets and replays. prefers-reduced-motion renders the final
 * populated state immediately. Styled for a dark #0a0f1c section. Illustrative content.
 */
  ...
  return (
    <div ref={rootRef}>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <p className="font-cv-mono text-[10.5px] font-semibold uppercase tracking-[0.18em] text-white/45">
            Different inputs. Same ranked output.
          </p>
          <button
            type="button"
            onClick={play}
