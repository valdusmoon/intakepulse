...
## 13.3 Live re-test #3 (2026-07-23, ~1:59am ET) — call d0e4d48c / lead a3d4b82c

**PASS:** one-shot opener extracted service+urgency ("fire restoration... it's an emergency"); mis-heard 6-digit ZIP "076401" correctly rejected → asked once → "07641" accepted; no name/callback questions; keypadless 61s call; scored 83 Hot + summary written ~9s after lead creation (Inngest fired from INSIDE the live call at 05:58:59, `finalizeVoiceCall: complete` 05:59:07, duplicate trigger no-op'd). Durable-finalize architecture verified end-to-end in prod.

**Open issues found this round:**
- [x] **Goodbye clip round 3 — INSTRUMENTED + STRUCTURAL FIX (2026-07-23, awaiting next live call to verify).** Per-call diagnostics added to the stream route + openai-handler: audio_chunk marks sent vs echoed, goodbye mark-send→echo latency, estimatedUnplayedMs at echo/teardown, a warn when the backstop timer fires without the echo, a log when Twilio's `stop` lands mid-goodbye, a state-tagged log on every `clear` sent, and one summary line per call with `closedBy` (mark-echo | backstop-timer | twilio-stop | ...). External briefing at `docs/goodbye-clip-brief.md`; ChatGPT review confirmed H1 with the mechanism: our `<Connect>` had NO `action`, so WS close = TwiML document ends = Twilio hangs up that instant, cutting the audio tail still in carrier buffers even after the mark echo. **Fix shipped: `<Connect action>` → new `/api/twilio/voice/after-stream` route (signature-verified) returning `<Pause length="1"/><Hangup/>`** — Twilio's call-control layer owns teardown; the route logs per call so we can see the guard ran. Mark echo stays the primary close signal, byte-math timer stays backstop-only. Rejected from the review: keying off `response.output_audio.done` instead of `response.done` (it fires EARLIER, gains nothing; wait is on playout).
- [x] **Bell/new-leads badge fixed (2026-07-23).** `getNewLeadsCount` + the bell dropdown now count/list pre-contact jobs (`leadStatus IN ('new','qualified')` — exported as `PRE_CONTACT_STATUSES`), so a scored Hot lead stays on the bell until it's actually contacted. `getLeadsByBusiness` accepts `leadStatus: string | string[]`. (Deeper "acknowledged/viewed" marker still an option later if new+qualified proves too broad.)
- [x] **Push stale-endpoint hardening SHIPPED (2026-07-23), except device list.** (a) `pushsubscriptionchange` handler added to sw.js — re-subscribes using the old subscription's own applicationServerKey and POSTs `{subscription, oldEndpoint}`; subscribe route now deletes the rotated-away row. (b) `syncLocalSubscription()` silently re-POSTs the live local subscription once per page load from `usePushNotifications.refresh()` (runs on dashboard home via NotificationsPrompt and in Settings). (c) Lead pushes now sent with `urgency: "high"` + TTL 1h. (d) Every fan-out logs `push: lead notification fan-out {devices, delivered, pruned, failed}` — no more silent 201s. Remaining: (e) Settings device list — deferred; needs a label-on-subscribe design because UA can't distinguish phone vs desktop (desktop-site mode sends identical UA).
- [x] **Two zombie test leads from the pre-fix broken calls** — RESOLVED 2026-07-23: deleted from prod (Bob 0c673bfb, "Dane" fbecfb9c); their 2 call records kept but detached (`lead_id` nulled). Verified zero unscored new/qualified job leads remain — badge count is clean.

## 14. Final launch validation — the two remaining live tests (added 2026-07-23, post priority_v2)

- [ ] **AI voice test (priority_v2 live validation)** — one AI-answered call to the prod number. Confirm: full goodbye plays (verifies the `<Connect action>` fix via the goodbye-diagnostics log line + `/api/twilio/voice/after-stream` log); lead scores on the NEW standard — fire emergency should land ~96 Hot with qualityScore 65 (service+urgency+ZIP) and `score_trace.version: "priority_v2"` with `valueSource` recorded; push + email both arrive (email now goes to the real owner address). Bonus: one off-list call ("I've got a pest problem") to see `valueSource: "ai_relative"` with a price-list-anchored estimate instead of the flat base range.
- [ ] **Human recording test (team-answered capture)** — flip Blue Star back to `ring_then_ai`, ANSWER the call yourself with recording enabled. Confirm: disclosure plays, transcript + summary land on the call record (audio deleted), the call becomes a job or message per the transcript, NO alert fires (operator was on the call), and — new in v2 — if you quote a price on the call ("that'll run you about $800"), the lead's estimate stores YOUR number with `valueSource: "owner_quoted"`, not a benchmark.
- [x] Pre-req from 13.3: resolve the two zombie leads — DONE 2026-07-23 (deleted from prod, calls detached, badge verified clean).

## 15. Backlog — PARKED, post-launch only

- [ ] **AI chat widget (voice intake in text form, added 2026-07-23).** Same rigid state machine as voice in a text modality: model classifies into closed enums, code owns every transition, same 4-question set, same job/message/junk taxonomy, same priority_v2 scoring, approved-price-line-or-nothing. NOT a chatbot per the constitution — this amends §5 (new surface), not §1 (identity). Why it's cheap: the flow logic is modality-independent; drop the audio layer (no Twilio/barge-in/Whisper ZIP-mangling), keep the routing, `markMessage` seam carries over. Why it sells: Podium/Birdeye headline "webchat" — "same AI that answers your phone also answers your website." Design notes captured now so they're not lost: (a) no caller ID in chat, so phone number MUST be asked (like the web form's contact step); (b) mid-conversation abandonment is much higher than voice — save partial capture when the tab closes; (c) public unauthenticated endpoint making per-message model calls = spam/cost exposure the form doesn't have (rate limit hard, consider a session token per widget load); (d) embed versioning on customers' sites. **Trigger to build: a prospect asks for it on a cold call, OR form-conversion data shows the static form leaking. Do not build before launch.**
